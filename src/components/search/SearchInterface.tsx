...
export function SearchInterface({
  ...
  const [sortBy, setSortBy] = useState<
    'relevance' | 'name' | 'brand' | 'popularity' | 'created'
  >('relevance');
    ...
    async (query: string, page: number = 1) => {
      ...
      try {
        ...
        const result: SearchResultType = {
          ...
          facets: {
            brands: response.suggestions.brands.map(brand => ({
              value: brand,
              count: 0,
            })),
            categories: response.suggestions.categories.map(cat => ({
              value: cat,
              count: 0,
            })),
            priceRanges: [],
  ...
  const handleBarcodeDetected = useCallback(
    (result: BarcodeScanResult) => {
      const term =
        result.productInfo?.name?.trim() || result.code?.toString().trim();
      if (term) {
        setSearchQuery(term);
        void performSearch(term);
      } else {
        onError?.('Barcode detected, but no product information was found.');
      }
  ...
  const attachScanner = useCallback(
    async (facing: 'environment' | 'user') => {
      const videoElement = barcodeVideoRef.current;
      if (!videoElement) {
        throw new Error('Barcode video element is not ready');
      }
      ...
      await barcodeService.startScanning(
        ...
        handleBarcodeError,
        { video: videoElement, facingMode: facing, frequency: 12 }
      );
    },
    [handleBarcodeDetected, handleBarcodeError]
  );

  const startBarcodeSearch = useCallback(() => {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        await attachScanner(cameraFacing);
        if (!cancelled) {
          setSwitchingCamera(false);
        }
      } catch (error) {
  ...
  }, [showBarcodeScanner, cameraFacing, attachScanner, onError]);
  ...
  const handleSwitchCamera = useCallback(() => {
    if (!showBarcodeScanner || switchingCamera) {
      return;
    }
    setCameraFacing(prev => (prev === 'environment' ? 'user' : 'environment'));
  }, [showBarcodeScanner, switchingCamera]);
      ...
      <div className="mb-6">
        <div className="flex items-center space-x-4 mb-4">
          ...
          {showBarcode && (
            <button
              onClick={
                showBarcodeScanner ? stopBarcodeSearch : startBarcodeSearch
              }
              className={`p-3 rounded-lg border transition-colors ${
                showBarcodeScanner
                  ...
                  <div className="bg-white border rounded-lg p-4">
                    ...
                    <div className="space-y-2">
                      ...
                      {filters.brand?.map(brand => (
                        <div
                          key={brand}
                          className="flex items-center justify-between bg-green-50 px-2 py-1 rounded text-sm"
                        >
                      ...
                      {filters.verified !== undefined && (
                        <div className="flex items-center justify-between bg-purple-50 px-2 py-1 rounded text-sm">
                          <span>
                            Verified: {filters.verified ? 'Yes' : 'No'}
                          </span>
                          <button
                        ...
                        {searchResults.facets.brands.map(brand => (
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={
                                filters.brand?.includes(brand.value) || false
                              }
                              onChange={e => {
                    ...
                    >
                      ...
                      <div className={viewMode === 'list' ? 'flex-1' : ''}>
                        <div className="flex items-start justify-between mb-2">
                          ...
                          >
                            <Heart
                              className={`w-4 h-4 ${wishlist.has(supplement.id) ? 'fill-current' : ''}`}
                            />
                          </button>
                        </div>
          ...
          {!searchResults && !loading && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">
                Find detailed information about vitamins, minerals, and other
                supplements
              </p>
